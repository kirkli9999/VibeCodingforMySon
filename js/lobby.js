...
(() => {
    ...
    function showLobby() { lobbySection.style.display='flex'; waitingSection.style.display='none'; gameContainer.style.display='none'; }
    function showWaiting(text) { lobbySection.style.display='none'; waitingSection.style.display='flex'; gameContainer.style.display='none'; statusText.textContent=text; }
    function showGame() { lobbySection.style.display='none'; waitingSection.style.display='none'; gameContainer.style.display='block'; }

    practiceBtn.addEventListener('click', () => {
        isHost = true; showGame(); Renderer.init(canvas); startPracticeGame();
    });

    createBtn.addEventListener('click', async () => {
        createBtn.disabled = true; showWaiting('建立房間中...');
        try {
            const code = await NetworkManager.createRoom();
            roomCodeDisplay.textContent = code;
            showWaiting(''); statusText.textContent = '等待對手加入...';
            document.getElementById('code-container').style.display = 'block';
            isHost = true;
            NetworkManager.onConnect(() => startGame());
            NetworkManager.onDisconnect(() => { stopGame(); showWaiting('對手已斷線，等待重新連線...'); });
    ...
    joinBtn.addEventListener('click', async () => {
        ...
        joinBtn.disabled = true; showWaiting('連線中...');
        try {
    ...
    function startGame() {
        showGame(); Renderer.init(canvas);
        if (isHost) startHostGame(); else startGuestGame();
    }

    function startPracticeGame() {
        gameState = new GameState(); gameState.started = true;
        Controls.init((action, pressed) => gameState.players[0].applyInput(action, pressed));
    ...
    function startGuestGame() {
        latestState = null;
        Controls.init((action, pressed) => NetworkManager.send({ type: 'input', action, pressed }));
    ...
    function stopGame() {
        ...
        gameState = null; latestState = null;
    }
